...
def initialPythiasub(m, template_h, template_x, template_y, lag, lag_h, lag_x, lag_y, Cdatabase, cdatabase, zuobiaolist,
    ...
    while sqflag == 0:
        ...
        path1 = './output2/initial1' + str(order) + '.npy'
        np.save(path1, m2)
        ...
        path2 = './output2/initial2' + str(order) + '.npy'
        path3 = './output2/outputinitial4' + str(order) + '.vtk'
        np.save(path2, m2)
        ...
        if num_Seq>10:
            sqflag = 1
    m = cut(m2, lag)
    path4 = './output2/initial_zone' + str(order) + '.npy'
    np.save(path4,m)
    return m
...
def initialAIforPythia(m, template_h, template_x, template_y, lag, lag_h, lag_x, lag_y, N, U, hardlist, codelist,
    ...
    m[:a,:b//2,:c//2]=np.load('./output2/initial_zone1.npy')
    m[:a,:b//2,c//2:401]=np.load('./output2/initial_zone2.npy')
    m[:a,b//2:401,:c//2]=np.load('./output2/initial_zone3.npy')
    m[:a,b//2:401,c//2:401]=np.load('./output2/initial_zone4.npy')


    time_end = time.time()

    np.save('./output2/outputinitial3.npy', m)
